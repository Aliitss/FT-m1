...
Implementar la clase LinkedList, definiendo los siguientes métodos:
  ...
  coincida con lo buscado; en el segundo, buscamos un nodo cuyo valor, al ser pasado como 
...
LinkedList.prototype.search = function(value){
  ...
  if (typeof valueOrCB === "function"){
    if (valueOrCB(this.head.value)) return valueOrCB;
  } else {
    if (this.head.value === valueOrCB) return valueOrCB;
  }
  let current = this.head;
  while(current.next){
    current = current.next;
...
La clase debe tener los siguientes métodos:
  - hash: función hasheadora que determina en qué bucket se almacenará
   un dato. Recibe un input alfabético, suma el código numérico de 
   cada caracter del input (investigar el método charCodeAt de los 
    strings) y calcula el módulo de ese número total por la cantidad 
    de buckets; de esta manera determina la posición de la tabla en 
    la que se almacenará el dato.
  - set: recibe el conjunto clave valor (como dos parámetros distintos),
   hashea la clave invocando al método hash, y almacena todo el 
   conjunto en el bucket correcto.
  - get: recibe una clave por parámetro, y busca el valor que le 
...
Ejemplo: supongamos que quiero guardar {instructora: 'Ani'} en la 
tabla. Primero puedo chequear, con hasKey, si ya hay algo en la tabla
 con el nombre 'instructora'; luego, invocando set('instructora', 
 'Ani'), se almacenará el par clave-valor en un bucket específico 
...
HashTable.prototype.hash = function (key){
  ...
  for (let i = 0; i < key.length; i++){
    cuenta += key.charCodeAt(i);
  }
  return cuenta % this.numBuckets;
};
